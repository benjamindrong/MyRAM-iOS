...
final class SyncBatchUnsentQueueTests: XCTestCase {
    ...
    func testMYR229ReconnectBatchProcessesEmbeddedProviderBeforeDependent() throws {
        let noteID = UUID(uuidString: "00000000-0000-0000-0000-000000229001")!
        let deviceID = UUID(uuidString: "280CD3D7-6663-4881-B097-CD79AF5C88AF")!
        let providerID = SyncOperationID(deviceID: deviceID, localCounter: 265)
        let dependentID = SyncOperationID(deviceID: deviceID, localCounter: 266)
        let reconnectBaseline = SyncTextSequenceState.empty

        let provider = try SyncBatchAnchoredPayloadAdapter.makeInsertedChange(
            noteID: noteID,
            utf16Offset: 0,
            text: "01234567890",
            ...
            state: reconnectBaseline
        )
        guard case .noteBodyTextInsertedAnchored(let providerChange) = provider else {
            return XCTFail("Expected anchored provider")
        }
        let stateWithProvider = try SyncBatchAnchoredInsertReplay.applying(
            providerChange,
            to: reconnectBaseline
        ).sequenceState
        let dependent = try SyncBatchAnchoredPayloadAdapter.makeInsertedChange(
            noteID: noteID,
            utf16Offset: 11,
            text: "X",
        ...
        guard case .noteBodyTextInsertedAnchored(let dependentChange) = dependent else {
            return XCTFail("Expected anchored dependent")
        }
        XCTAssertEqual(
            dependentChange.payload.anchor,
            .after(try SyncTextElementID(operationID: providerID, elementOffset: 10))
        )

        // Reconnect/bootstrap established the valid shared baseline before these queued edits.
        // The retained batch is intentionally in the failure order observed by MYR-229:
        // dependent first, exact dependency provider later in the same durable batch.
        let batch = SyncBatch(
            id: UUID(uuidString: "00000000-0000-0000-0000-000000229002")!,
            originDeviceID: deviceID,
            createdAt: Date(timeIntervalSince1970: 2_292),
            batchSequence: 1,
            changes: [dependent, provider]
        )
        let baselineSnapshot = NoteSequenceStateMutationSnapshot(
            noteID: noteID,
            body: "",
            revision: 0,
            state: reconnectBaseline
        ...
        let outcome = try SyncConvergenceAnchoredBatchPlanner().plan(
            indexedChanges: Array(batch.changes.enumerated()).map { ($0.offset, $0.element) },
            batch: batch,
            expectedSnapshot: baselineSnapshot,
            recoverySnapshot: SyncBatchAnchoredRecoveryStoreSnapshot(records: [], health: .healthy)
        )

        guard case .success(let plan) = outcome else {
            return XCTFail("Exact provider in the retained batch must remain processable after reconnect")
        }
        XCTAssertEqual(plan.finalBody, "01234567890X")
        XCTAssertTrue(plan.recoveryTransitions.isEmpty)
    }
