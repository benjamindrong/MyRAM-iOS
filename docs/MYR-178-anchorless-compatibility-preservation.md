...
Remediation proof is split across production seams without inventing queue or ACK results: `MyRAMTests.testReentrantHashlessBatchDefersAcknowledgementWhileActiveDrainClassifiesIt` drives the shared runtime through the asynchronous active-presentation drain; `MyRAMSyncControllerTests` covers deferred/rejected/successful iPhone receive status and the existing manual resend path through sender retention, retransmission, ACK, and removal; `MacSyncBatchControllerTests` covers native Mac durable capture, rejection silence, successful receive ACK, and sender retention; and `SyncConvergencePlanningTests.testAlreadyDrainingDefersTransportAcknowledgementWithoutClassifyingCompatibility` pins the shared policy mapping. The existing ordinary/title-only reentrant drain test remains unchanged.
